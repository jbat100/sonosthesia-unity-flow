...
{
    public class DynamicChannel<T> : MonoBehaviour where T : struct
    {
        ...
        protected virtual void OnEnable()
        {
            ...
            if (_log)
            {
                _logSubscriptions.Add(StreamObservable.Subscribe(stream =>
                    _logSubscriptions.Add(stream.Subscribe(value => Debug.Log($"{this} stream emitted {value}"), 
                        e => Debug.LogError($"{this} stream error {e.Message}"), 
                        () => Debug.Log($"{this} stream completed")))
                    ));
            }
        }
